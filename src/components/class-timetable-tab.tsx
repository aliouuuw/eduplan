...
export default function ClassTimetableTab({
  ...
  // Fetch time slots and timetable
  const fetchData = async () => {
    try {
      // Fetch time slots for this class's template
      const timeSlotsResponse = await fetch(`/api/time-slots?classId=${classId}`);
      ...
      const timetableResponse = await fetch(`/api/timetables?classId=${classId}`);
      if (timetableResponse.ok) {
        const timetableData = await timetableResponse.json();
        const mappedEntries = (timetableData.timetable || []).map((entry: any) => ({
          id: entry.id,
  ...
  const handleCellClick = (timeSlotId: string, slot: TimeSlot) => {
    if (slot.isBreak || !classId) return;
      ...
      {activeDays.length > 0 && (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <Card className="rounded-2xl border border-gray-200 shadow-sm">
            ...
            <CardContent>
              <p className="text-3xl font-semibold text-black">{stats.activeDays.length}</p>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700 mt-1">
                  This class needs to be assigned to a time slot template before you can create a timetable.
                </p>
                <p className="text-sm text-yellow-700 mt-1">
                  Go to Scheduling → Templates to create or assign templates to classes.
                </p>
              </div>
            </div>
          </CardContent>
